...
bool ImGui::Keybind(const char* label, int* key, bool widget_label) {
    ...
    ImRect mode_rect(window->DC.CursorPos + ImVec2(size - 0, 00), window->DC.CursorPos + ImVec2(size, 40));
    ImRect key_rect;
    ...
    if (widget_label)
    window->DrawList->AddText(rect.Min + ImVec2(0, 0), GetColorU32(colors::Text), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(colors::Text));

    window->DrawList->AddRectFilled(key_rect.Min + ImVec2(CalcItemWidth() - 5, -1), key_rect.Max + ImVec2(12, -12), GetColorU32(colors::keybind_background), 7);
    window->DrawList->AddRect(key_rect.Min + ImVec2(CalcItemWidth() - 5, -1), key_rect.Max + ImVec2(12, -12), GetColorU32(colors::keybind_border), 7);

    RenderTextClipped(key_rect.Min - ImVec2(20, 31), key_rect.Max + ImVec2(CalcItemWidth() + 28, 16), buf_display1, NULL, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::TabEx(const char* ic0, const char* label, bool selected, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderTextClipped(bb.Min + ImVec2(15, 0), bb.Max + ImVec2(15, 0), ic0, NULL, &label_size, style.ButtonTextAlign, &bb);
    PopFont();

    RenderTextClipped(bb.Min + ImVec2(50, 1), bb.Max + ImVec2(50, 1), label, NULL, &label_size, style.ButtonTextAlign, &bb);
    PopStyleColor();
    PopStyleVar(1);

    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(50.000f, 35.000f), bb.Max + ImVec2(10, 4.000f), GetColorU32(colors::Tab_Border));
...
bool ImGui::Tab(const char* ic0, const char* label, bool selected, const ImVec2& size_arg)
{
    return TabEx(ic0, label, selected, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.check_paint = ImLerp(it_anim->second.check_paint, *v ? 15.0f : 0.0f, g.IO.DeltaTime * 5.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background_color), false, style.FrameRounding);


    PushClipRect(total_bb.Min + ImVec2(4, 4), total_bb.Min + ImVec2(4 + it_anim->second.check_paint, 27), true);
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(4, 4), GetColorU32(it_anim->second.check_mark), 12);
    PopClipRect();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        ...
        window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2 + 34, bb.Max.y), GetColorU32(it_anim->second.background_color), style.FrameRounding);

        window->DrawList->AddRectFilled(bb.Min + ImVec2(155, 0), ImVec2(value_x2 - 3, bb.Max.y + 1), GetColorU32(ImGuiCol_ChildBg), 0);
    ...
    {
        RenderTextClipped(bb.Min + ImVec2(5, 5) + style.FramePadding, ImVec2(value_x2 + 20, bb.Max.y), preview_value, NULL, NULL);
        ImRotateStart();
        PushFont(ico_minimize);
        RenderTextClipped(bb.Min + ImVec2(w + 14, 0) + style.FramePadding, ImVec2(value_x2 + 25, bb.Max.y), "I", NULL, NULL);
        PopFont();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + 53, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 14), ImVec2(total_bb.Min.x + w + 53, total_bb.Min.y + 20), GetColorU32(it_filing->second.background_color), 30);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        SetCursorPos(GetCursorPos() - ImVec2(38, 0));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    float grid_step = ImMin(size.x, size.y) / 2.55f;
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min + ImVec2(CalcItemWidth() + 30, 4), bb_inner.Max - ImVec2(0, 4), GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
        else
            window->DrawList->AddRectFilled(bb_inner.Min + ImVec2(CalcItemWidth() + 30, 4), bb_inner.Max - ImVec2(0, 4), GetColorU32(col_source), rounding);
    }
